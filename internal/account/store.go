...
// `make generate`(go generate ./...) 실행 후에만 컴파일된다.
// Ent API 세부 명칭은 생성 결과에 맞춰 미세 조정될 수 있다.
...
// Store 는 account 영속 구현(Ent). Repository 인터페이스를 만족한다.
// 돈이 걸린 핵심 경로는 Ent 트랜잭션(client.Tx) 또는 raw SQL로 명시 제어.
type Store struct {
	client *ent.Client
}

// NewStore 는 Store를 만든다.
...
// MarkDeployed 는 배포 완료로 표시한다. 동일 값 재적용도 무해(멱등).
